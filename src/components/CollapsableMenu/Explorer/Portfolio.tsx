      ...
      {expanded && (
        <>
          ...
          {/* <Folder name="public" openIcon={<PublicOpen />} closedIcon={<Public />} indent={0} segmentActive={segments.length === 0}> */}
            <File name="About Me" icon={<FavIcon />} url="/" indent={0} sections={pathname === '/' ? sections : []} />
          {/* </Folder> */}
          {/* <Folder name="src" openIcon={<SrcOpen />} closedIcon={<Src />} indent={0} segmentActive={false}> */}
            <Folder name="My Work" openIcon={<AppOpen />} closedIcon={<App />} indent={0} segmentActive={segments[0] === 'apps'}>
              {allApps.map((app) => (
                <File key={app.pathname} name={app.title} icon={fileType[app.framework]} url={app.pathname} indent={1} sections={pathname === app.pathname ? sections : []} />
              ))}
            </Folder>
            {/* <Folder name="Leetcode" openIcon={<LibOpen />} closedIcon={<Lib />} indent={0} segmentActive={segments[0] === 'leet-code'}>
              {allLeetcode.map((leetcode) => (
                <File key={leetcode.pathname} name={leetcode.title} icon={fileType[leetcode.framework]} url={leetcode.pathname} indent={1} sections={pathname === leetcode.pathname ? sections : []} />
              ))}
            </Folder> */}
          {/* </Folder> */}
